		...
		<div class="container">
			<?php
				...
				if ($_POST['submit'] == "import") {
					if (isset($_FILES["cus_data"])) {
						$file_name = $_FILES['cus_data']['name'];
						$file_tmp = $_FILES["cus_data"]["tmp_name"];
						$file_type = $_FILES["cus_data"]["type"];
						$file_type_substr = substr($file_name, -4);
						
						$target_file_name = "Cus_Uploads_" . date("Y-m-d_His") . "_" . $file_name;
						...
						$sql_del_upload = "delete from cus_mstr ";
						...
						for ($row_count_sheet = 0; $row_count_sheet < $sheetCount; $row_count_sheet++) // loop in each sheet to Insert into Database differenct Order Supplier Country
						{
							...
							if (trim($result["A"]) != "") {	
								$cus_nbr = CheckString($result["A"]);
								$cus_name1 = CheckString(str_replace('/[^a-z0-9\_\- ]/i', '',$result["B"]));
								$cus_name2 = CheckString(str_replace('/[^a-z0-9\_\- ]/i', '',$result["C"]));
								$cus_name3 = CheckString($result["D"]);
								$cus_name4 = CheckString($result["E"]);
								$cus_street = CheckString(str_replace('/[^a-z0-9\_\- ]/i', '',$result["F"]));
								$cus_street2 = CheckString(str_replace('/[^a-z0-9\_\- ]/i', '',$result["G"]));
								$cus_street3 = CheckString($result["H"]);
								$cus_street4 = CheckString($result["I"]);
								$cus_street5 = CheckString($result["J"]);
								$cus_district = CheckString($result["K"]);
								$cus_city = CheckString($result["L"]);
								$cus_zipcode = CheckString($result["M"]);
								$cus_country = CheckString($result["N"]);
								$cus_region = CheckString($result["O"]);
								$cus_search_term = CheckString($result["P"]);
								$cus_version = CheckString($result["Q"]);
								$cus_tax_nbr3 = CheckString($result["R"]);
								$cus_tax_nbr4 = CheckString($result["S"]);
								$cus_com_code = CheckString($result["T"]);
								$cus_recon_acc = checkstring($result["U"]);
								$cus_terms_paymnt = CheckString($result["V"]);
								$cus_acc_group = CheckString($result["W"]);
								$cus_create_by = $user_login;
								$cus_create_date = date("Y-m-d H:i:s");
								$cus_stamp_date = date("d/m/Y");
								$cus_update_by = '';
								$cus_update_date = '';
								
								// $sql = "SELECT cus_nbr FROM cus_mstr WHERE cus_nbr='" . $result["A"] . "' ";  // Nanthawat : Select the cells you want to check for duplicates.
								
								// $result_add = sqlsrv_query($conn, $sql);
								
								// $row = sqlsrv_fetch_array($result_add, SQLSRV_FETCH_ASSOC);
								// if ($row <= 0) {
									$sql_ins_ordm = "insert into cus_mstr ([cus_nbr],[cus_name1],[cus_name2]
									,[cus_name3]
									,[cus_name4]
									,[cus_street]
									,[cus_street2]
									...
									"values('" . $cus_nbr . "','" . $cus_name1 . "','" . $cus_name2 . "','"
									. $cus_name3 . "','"
