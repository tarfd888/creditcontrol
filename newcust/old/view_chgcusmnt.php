...
if($rowCounts > 0){
	while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC))
	{
		...
		$cust_code = mssql_escape($row['cus_code']);
    ...
    if($result) {
        while($rows = sqlsrv_fetch_array($result)) {						
            ...
            if(isset($rows['apprv_date'])){
                ...
                //$apprv_date = date_format($rows['apprv_date'], "d/m/Y H:i:s");
                if($rows['apprv_status']=="AP"){$apprv_status ="<span style='color:green'><strong>*** Approved ***</strong></span>";}
			}
