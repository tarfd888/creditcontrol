...
if($rowCounts > 0){
	while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC))
	{
		...
		// $cus_prov = html_clear($row['cus_prov']);
		// $cus_zip = html_clear($row['cus_zip']);
        ...
        $cus_contact1_nme = html_clear($row['cus_contact1_nme']);
        $cus_contact1_pos  = html_clear($row['cus_contact1_pos']);
        $cus_contact2_nme = html_clear($row['cus_contact2_nme']);
        $cus_contact2_pos  = html_clear($row['cus_contact2_pos']);
        ...
        // -- page 2 --//
		$cus_term = html_clear($row['cus_term']);
		$cus_bg1 = html_clear($row['cus_bg1']);
		$cus_cr_limit1 = CheckandShowNumber(html_clear($row['cus_cr_limit1']),2);
		$cus_bg2 = html_clear($row['cus_bg2']);
		$cus_cr_limit2 = CheckandShowNumber(html_clear($row['cus_cr_limit2']),2);
		$cus_cond_term = html_clear($row['cus_cond_term']);
        $cus_cond_term_oth = html_clear($row['cus_cond_term_oth']);    
		$cus_pay_addr = html_clear($row['cus_pay_addr']);
		$cus_contact_nme_pay = html_clear($row['cus_contact_nme_pay']);
		$cus_contact_tel = html_clear($row['cus_contact_tel']);
		$cus_contact_fax = html_clear($row['cus_contact_fax']);
		$cus_contact_email = html_clear($row['cus_contact_email']);    
...
if($rowCounts > 0){
	...
	{
        // -- page 3 --//
        $cusd_tg_beg_date = html_clear(dmytx($row['cusd_tg_beg_date']));
        $cusd_tg_end_date = html_clear(dmytx($row['cusd_tg_end_date']));
        $cusd_sale_est = CheckandShowNumber(html_clear($row['cusd_sale_est']),2);
        $cusd_sale_vol = CheckandShowNumber(html_clear($row['cusd_sale_vol']),2);

        $cusd_obj1 = html_clear($row['cusd_obj1']);
        $cusd_obj2 = html_clear($row['cusd_obj2']);
        $cusd_obj3 = html_clear($row['cusd_obj3']);
        ...
        $cusd_cust_prop1 = html_clear($row['cusd_cust_prop1']);
        $cusd_cust_prop2 = html_clear($row['cusd_cust_prop2']);
        $cusd_cust_prop3 = html_clear($row['cusd_cust_prop3']);
        ...
        $cusd_aff1 = html_clear($row['cusd_aff1']);
        $cusd_aff2 = html_clear($row['cusd_aff2']);
        $cusd_aff3 = html_clear($row['cusd_aff3']);
        ...
        $cusd_dealer1_nme = html_clear($row['cusd_dealer1_nme']);
        $cusd_dealer1_avg_val = CheckandShowNumber(html_clear($row['cusd_dealer1_avg_val']),2);
        $cusd_dealer2_nme = html_clear($row['cusd_dealer2_nme']);
        $cusd_dealer2_avg_val = CheckandShowNumber(html_clear($row['cusd_dealer2_avg_val']),2);
        $cusd_dealer3_nme = html_clear($row['cusd_dealer3_nme']);
        $cusd_dealer3_avg_val = CheckandShowNumber(html_clear($row['cusd_dealer3_avg_val']),2);
        ...
        $cusd_comp1_nme = html_clear($row['cusd_comp1_nme']);
        $cusd_comp1_avg_val = CheckandShowNumber(html_clear($row['cusd_comp1_avg_val']),2);
        $cusd_comp2_nme = html_clear($row['cusd_comp2_nme']);
        $cusd_comp2_avg_val = CheckandShowNumber(html_clear($row['cusd_comp2_avg_val']),2);
        $cusd_comp3_nme = html_clear($row['cusd_comp3_nme']);
        $cusd_comp3_avg_val = CheckandShowNumber(html_clear($row['cusd_comp3_avg_val']),2);
...
$sql_stamp = "select * FROM apprv_person Where apprv_cus_nbr = ?  order by apprv_seq,apprv_id asc";
    ...
    $params = array($q);
    $sql_last = "SELECT top 1 * FROM cus_approval where cus_ap_nbr= ? order by cus_ap_id desc";
        ...
        <div class="card">
            <div class="card-header">
                <div class="heading-elements">
                    ...
                    <?php if (inlist("11,21,51",$cus_step_code)) {  ?>
                        <button id="btnshowtext" class="btn btn-sm btn-outline-warning btn-min-width btn-glow mr-1 mb-1"><i class="fa fa-bell"></i> มีข้อความถึงคุณ</button>
                    <?php } ?>    
            ...
            <div class="card-content">
                <div class="card-body">
                    <form id="frm_cust_add" name="frm_cust_add" autocomplete=OFF method="POST" enctype="multipart/form-data"
                        ...
                        <input type="hidden" name="rem_revise" id="rem_revise"
                        value="<?php echo $rem_revise ?>">
                        <input type="hidden" name="cus_whocanread" id="cus_whocanread" value="<?php echo $cus_whocanread ?>">
                        ...
                        <ul class="nav nav-tabs nav-linetriangle nav-justified">
                            ...
                            <li class="nav-item">
                                <a class="nav-link <?php echo $active1; ?>" id="helpIcon21-tab1" data-toggle="tab" href="#helpIcon21"
                        ...
                        <div class="tab-content px-1 pt-1 font-small-2">
                            ...
                            <div role="tabpanel" class="tab-pane <?php echo $active; ?>" id="activeIcon22"
                                ...
                                <?php include("upd_newcusmnt_tab1.php"); ?>
                            ...
                            <div class="tab-pane <?php echo $active; ?>" id="profileIcon22" role="tabpanel"
                                ...
                                <?php include("upd_newcusmnt_tab2.php"); ?>
                            ...
                            <div class="tab-pane <?php echo $active; ?>" id="aboutIcon21" role="tabpanel"
                                ...
                                <?php include("upd_newcusmnt_tab3.php"); ?>
                            ...
                            <div class="tab-pane <?php echo $active1; ?>" id="linkIcon21" role="tabpanel"
                                ...
                                <?php include("upd_newcusmnt_tab4.php"); ?>
                            ...
                            <div class="tab-pane <?php echo $active; ?>" id="helpIcon21" role="tabpanel"
                                ...
                                <?php include("upd_newcusmnt_tab5.php"); ?>
                                <!-- <p>Cookie icing tootsie roll cupcake jelly-o sesame snaps. Gummies cookie dragée cake jelly
									marzipan donut pie macaroon. Gingerbread powder chocolate cake icing. Cheesecake gummi
									bears ice cream marzipan.</p> -->
                            </div>
                            ...
                            <div class="form-actions right showBtnSave" style="display:none">
                                ...
                                <?php if(($can_editing) && ($cus_step_code=="0")) { ?>
                                    <?php if($apprv_id_array_count == 1) {?>
                                        <button type="button" id="btnsale_sub" name="btnsale_sub" class="btn btn-outline-info btn-min-width btn-glow mr-1 mb-1" style="display:show;" onclick="salesubmit('frm_submit','<?php echo encrypt('10', $key);?>','<?php echo $cus_app_nbr; ?>')"><i class="fa fa-paper-plane"></i> Submit</button> <!--Submit to Credit--> 
                                    <?php } else { ?>
                                        <button type="button" id="btnsale_sub" name="btnsale_sub" class="btn btn-outline-info btn-min-width btn-glow mr-1 mb-1" style="display:show;" onclick="salesubmit('frm_reviewer','<?php echo encrypt('40', $key);?>','<?php echo $cus_app_nbr; ?>')"><i class="fa fa-envelope"></i> Submit </button> <!--Submit to Reviewer-->
                                    <? } ?> 
                                <? } ?>           

                                <?php if(($can_editing) && ($cus_step_code=="21")) { ?>  <!-- Revising by Credit --> 
                                    <button type="button" id="btnsale_sub" name="btnsale_sub" class="btn btn-outline-info btn-min-width btn-glow mr-1 mb-1" style="display:show;" onclick="salesubmit('frm_revise_cr','<?php echo encrypt('20', $key);?>','<?php echo $cus_app_nbr; ?>')"><i class="fa fa-spinner"></i> Submit </button> <!--Submit to Credit-->
                                <? } ?> 
                                <?php if(($can_editing) && ($cus_step_code=="51") || ($cus_step_code=="52")) { ?>  <!-- Revising by SEC --> 
                                    <button type="button" id="btnsale_sub" name="btnsale_sub" class="btn btn-outline-info btn-min-width btn-glow mr-1 mb-1" style="display:show;" onclick="salesubmit('frm_revise_rev','<?php echo encrypt($rev_nextstep_code, $key);?>','<?php echo $cus_app_nbr; ?>')"><i class="fa fa-spinner"></i> Submit </button> <!--Submit for Revise-->
                                <? } ?> 
                                    <button type="button" id="btnsave" name="btnsave" value="add"
