<?php
	...
	$author_email = html_escape($_POST['author_email']);	
	...
	switch ($author_code) {
		...
		if($action_tiles == "1"){
			$fle = "author_mstr";
			$author_group = "Tiles";
			if($financial_amt_beg <= 3000001){ // ตำแหน่งตั้งแต่ ผฝ. ลง $author_position ค่าว่าง
				$author_position = "";
			}
		}else{
			$fle = "author_g_mstr";
	...
	if (!$errorflag) {
		...
		$sql_add = " INSERT INTO $fle (" . 
			" author_sign_nme,author_email,author_salutation,author_text, ".
...
if ($action == "add-remark") {
	if (!$errorflag) {
			...
			$sql_edit = "UPDATE author_g_mstr SET " .
