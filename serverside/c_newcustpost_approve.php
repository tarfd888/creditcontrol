...
if($cr_mgr_status=="A"){
	...
	$via_cr = "ไม่เห็นควรอนุมัติ";
}
if (inlist("cr_submit_app",$action)) {	
	if($cr_mgr_remark == "") {
		if ($errortxt!="") {$errortxt .= "<br>";}
		$errorflag = true;					
		$errortxt .= "กรุณาระบุ - [ ความคิดเห็น Finance & Credit Manager ]";
	}
}
// -- ผจก. ส่งอีเมลหาผู้อนุมัติ -- //
if ($action == "cr_submit_app") {
	////
	if (!$errorflag) {
		//Send Mail
		...
		switch($cus_cond_cust){
			...
			if($result_apprv) {
				...
				"cus_step_code = '$cr_step_code', ". ///// '61' Wait Department Approved
				...
				if($result_cr){
					...
					if($cr_step_code =="50") {$cr_step_code = "60";}
					$cr_ap_f_step = "30";  // FinCR Mgr 
					$cr_ap_t_step = $cr_step_code;
					$cr_ap_color = "text-info";
					$cr_ap_text = "Finance & Credit Manager ".$via_cr;
					//$cr_ap_text = "Finance & Credit Manager "."อนุมัติ";
					...
					" VALUES('$cr_ap_id','$cr_app_nbr','$cr_ap_f_step','$cr_ap_t_step','$cr_ap_text','$cr_mgr_remark','$cr_ap_color','1','$user_login','$today')";				
