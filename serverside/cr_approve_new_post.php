...
$action_post = html_escape($_POST['action']);
	if ($action_post != "") { //post มาจาก form 
		...
		$crnew_approve_mail =  html_escape($_COOKIE['crnew_approve_mail']);
		$sessionid_dec = decrypt($crnew_approve_mail, $key);
		if ($sessionid != $sessionid_dec) {
			setcookie ("crnew_approve_mail", "", time()-100,$ck_path,$ck_dom,$ck_secure,$ck_httponly);
			$r="0";
