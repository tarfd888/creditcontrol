<?php
	...
	//$formid = html_escape(decrypt($formid, $key));
	
	//--1. Parameter From approve.php
	$crstm_nbr = html_escape($_POST['crstm_nbr']);
	$crstm_cus_name = html_escape($_POST['crstm_cus_name']);
	$crstm_cus_nbr = html_escape($_POST['cr_cust_code']);
	
	$crstm_pre_yy = html_escape($_POST['crstm_pre_yy']);	
	$crstm_otd_pct = html_escape($_POST['crstm_otd_pct']);
	$crstm_ovr_due = html_escape($_POST['crstm_ovr_due']);
	$crstm_etc = html_escape($_POST['crstm_etc']);
	
	$crstm_cur_yy = html_escape($_POST['crstm_cur_yy']);	
	$crstm_otd1_pct = html_escape($_POST['crstm_otd1_pct']);
	$crstm_ovr1_due = html_escape($_POST['crstm_ovr1_due']);
	$crstm_etc1 = html_escape($_POST['crstm_etc1']);
	
	$crstm_ins = html_escape($_POST['crstm_ins']);
	...
	$crstm_detail_mail = html_escape($_POST['crstm_detail_mail']);
	...
	if (inlist("crctrlapp_edit",$action)) {	
		...
		if ((($crstm_mgr_reson=="") && inlist($user_role,"FinCR Mgr")) && inlist('40,41',$crstm_step_code)) {
				if ($errortxt!="") {$errortxt .= "<br>";}
				$errorflag = true;					
				$errortxt .= "กรุณาระบุ - [--- ความเห็น Manager  --- ]";
		}
		if ((($crstm_cr_mgr=="") && inlist($user_role,"FinCR Mgr")) && inlist('40,41',$crstm_step_code)) {
				if ($errortxt !="") {$errortxt .= "<br>";}
				$errorflag = true;
				$errortxt .= "กรุณาระบุ -[--- ยอดเงินอนุมัติ ---]";
		}
		if ((($crstm_mgr_rdo=="") && inlist($user_role,"FinCR Mgr")) && inlist('40,41',$crstm_step_code)) {
				if ($errortxt!="") {$errortxt .= "<br>";}
				$errorflag = true;
	...
	if ($action == "crctrlapp_edit") {
		...
		if ($del_cr2=="1") {
			...
			//if (inlist('20',$crstm_step_code) && inlist($user_role,'Action_View2')) {
			...
			//if (inlist('30,41',$crstm_step_code) && inlist($user_role,'FinCR Mgr')) {
