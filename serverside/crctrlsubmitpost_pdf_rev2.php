...
$crstm_nbr = html_escape($_POST['crstm_nbr']);	
	...
	$sql_cr = "SELECT * from crstm_mstr where crstm_nbr = ? and (crstm_step_code = '30' or crstm_step_code = '220' or crstm_step_code = '222')";
	$result_cr = sqlsrv_query($conn, $sql_cr,$params);	
	$r_cr = sqlsrv_fetch_array($result_cr, SQLSRV_FETCH_ASSOC);	
	if ($r_cr) {		
		$rc_curprocessor_check = $r_cr['crstm_reviewer2'];
			$allow_post = true;
	}
	...
	if (!$allow_post) {
		...
		"crstm_mstr.crstm_cr_mgr, crstm_mstr.crstm_cc_date_beg, crstm_mstr.crstm_cc_date_end, crstm_mstr.crstm_cc_amt,crstm_mstr.crstm_detail_mail, crstm_mstr.crstm_mgr_reson, crstm_mstr.crstm_reviewer2  ".
		...
		$r_cr = sqlsrv_fetch_array($result_detail, SQLSRV_FETCH_ASSOC);
			if ($r_cr) {
				...
				$cr_next_curprocessor_email =  html_clear($r_cr['crstm_reviewer2']);
				...
				if($crstm_cus_active=="1") { // เช็คลูกค้าเก่าหรือไม่
					array_push($fileattach,$output_folder.printMailapp_rev2($crstm_nbr,true,$output_folder,$cr_output_filename1,$conn));
					...
					array_push($fileattach,$output_folder.printMailapp_rev2_new($crstm_nbr,true,$output_folder,$cr_output_filename1,$conn));
					array_push($fileattach_mailname,$crstm_nbr."-เหตุผลที่เสนอขอวงเงิน.pdf");
				...
				//SEND EMAIL
				if ($crstm_step_code=="220") {  // ส่ง Mail ให้ Reviewer2 ตรวจสอบและอนุมัติ
					//ส่งหาผู้อนุมัติเลย
					if ($cr_next_curprocessor_email != "") {
						$approve_url = "<a href='".$app_url."/crctrlbof/cr_verify_mail_rev2.php?id=".encrypt($cr_next_curprocessor_email, $dbkey)."&nbr=".encrypt($crstm_nbr, $dbkey)."&act=".encrypt('221', $dbkey)."' target='_blank'><font color='green'> Approve</font></a>";
						//$revise_url = "<a href='".$app_url."/crctrlbof/cr_verify_mail_rev2.php?id=".encrypt($cr_next_curprocessor_email, $dbkey)."&nbr=".encrypt($crstm_nbr, $dbkey)."&act=".encrypt('222', $dbkey)."' target='_blank'><font color='blue'>Revise</font></a>";
						$reject_url = "<a href='".$app_url."/crctrlbof/cr_verify_mail_rev2.php?id=".encrypt($cr_next_curprocessor_email, $dbkey)."&nbr=".encrypt($crstm_nbr, $dbkey)."&act=".encrypt('223', $dbkey)."' target='_blank'><font color='red'> Reject </font></a>";
						$doc_url  = "<a href='".$app_url."/index.php><img src='_images/spacer.gif'></a>";
						$doc_bot =" <a href='javascript:void(0)'></a>";
						//$doc_url = "<a href='".$app_url."/index.php?doc=".encrypt($rc_nbr, strtoupper($cr_next_curprocessor_email))."&auth=".encrypt($rc_next_curprocessor, $dbkey)."' target='_blank'>คลิ๊กเพื่อเปิดเอกสารจากระบบ HR Recruitment System</a>";
					...
					if($email_sale != ""){
						...
						$mail_to =  $email_sale;
						$mail_subject = "ใบขออนุมัติวงเงิน :" ." $crstm_nbr ". " ลูกค้า "." $crstm_cus_name ". "ถูกส่งให้ผู้พิจารณา 2 เรียบร้อยแล้ว รอผลการตรวจสอบ";

						$mail_message =	"<font style='font-family:Cordia New;font-size:19px'>เรียน $name_from <br>
						<br>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; ใบขออนุมัติวงเงิน : $crstm_nbr ลูกค้า $crstm_cus_name ถูกส่งให้ผู้พิจารณา 2 เรียบร้อยแล้ว รอผลการตรวจสอบ <br><br>
						ขอบคุณค่ะ</font>";
